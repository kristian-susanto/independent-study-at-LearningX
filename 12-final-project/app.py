...
def home():
    ...
    try:
        ...
        # Menghitung total pembelian dan total buket hanya jika statusnya 'diterima'
        ...
        total_buket = sum(payment['quantity'] for payment in payments)
        total_pembelian_rupiah = format_rupiah(total_pembelian)

        return render_template('user/dashboard.html', title = title, user_info = user_info, total_pembelian = total_pembelian, total_buket = total_buket, total_pembelian_rupiah = total_pembelian_rupiah)
    except (jwt.ExpiredSignatureError, jwt.exceptions.DecodeError):
        title = 'Beranda'
...
def order_history():
    ...
    try:
        ...
        for transaction in transactions:
            ...
            bouquet = db.bouquets.find_one({'_id': ObjectId(transaction['bouquet_id'])})
            if bouquet:
                transaction['name_of_the_bouquet'] = bouquet.get('name', 'Buket tidak dikenal')
            else:
                transaction['name_of_the_bouquet'] = 'Buket tidak ditemukan'
...
def admin_home():
    ...
    return render_template('admin/dashboard.html', title = title, users_count = users_count, bouquets_count = bouquets_count, transactions_count = transactions_count, testimonials_count = testimonials_count, income = income, faqs_count = faqs_count, contact_count = contact_count)
...
def admin_transaction():
    ...
    for transaction in transactions:
        ...
        if bouquet:
            transaction['name'] = bouquet.get('name', 'Buket tidak dikenal')  # menangani nama buket yang tidak ada
        else:
            transaction['name'] = 'Buket tidak ditemukan'  # menangani data buket yang hilang
